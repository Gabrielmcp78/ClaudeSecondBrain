...
<!-- kb-status: level=active-focus | phase="Build Phase — platform codebase scaffolded & build-verified" | updated=2026-07-26 -->
**Status:** 🟢 Active — Current Focus · Build Phase (platform codebase scaffolded + build-verified) · Last updated: 2026-07-26 (Claude, Cowork — live Trello re-pull of Tech Guts/Tech Production/Ops & Setup + direct codebase infra audit; see Release Engineering Gap below for what's newly confirmed)

**Sourcing note (per Daily Keeper Protocol §3):** Trello is authoritative for task/decision/assignment state — always live-pull it, never trust this wiki's card-status prose past its "Last updated" date above. This wiki is authoritative for cross-domain synthesis (funding strategy reasoning, the pricing-conflict flag, connections to other projects) that has no single external system of record.
...
**Re-verified 2026-07-26 (Claude, Cowork session) — the gap is now confirmed at the file level, not just by card language:** `.env.local`'s `DATABASE_URL`/`DIRECT_URL` are literal `postgresql://postgres:****@[HOST]:.../postgres` placeholders — no Supabase project has ever been created, and there is no `prisma/migrations/` directory at all (migrations have never run once, anywhere). `.github/workflows/` does not exist. Neither `.env.staging` nor `.env.liveverify` exist — only `.env.local` and `.env.example`. All four Tech Guts cards opened 2026-07-16 (Pilot Environment and End-to-End Verification, Release Engineering CI and Quality Gates, Platform Delivery State: Source of Truth, Command Center — Live Operations Dashboard) remain unassigned as of this pull. Two build-related commits landed since the 2026-07-24 sync and are not yet reflected above: episode content management + publishing API/slug utilities (2026-07-25) and a new `mcp-server/` directory exposing this codebase to ChatGPT via MCP, plus a companion `CHATGPT_SYSTEM_PROMPT.md` (2026-07-22) — neither changes any locked architecture decision, both are additive. Full findings and an ambitious same-day action list (CI workflow + env-tier scaffolding buildable today with zero new accounts, vs. Supabase/Stripe/Clerk setup requiring Gabriel's hands) written to `outputs/2026-07-26_shakespearience-morning-status-and-todo.md`.
